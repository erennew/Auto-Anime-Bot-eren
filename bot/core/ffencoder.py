...
class FFEncoder:
    def __init__(self, message, path, name, qual):
        ...
        self.out_path = ospath.join("encode", f"{name}_{qual}p.mkv")  # Include quality in filename
        self.__prog_file = f'prog_{int(time())}.txt'  # Unique progress file
...
<blockquote>‣ <b>Files Encoded:</b> {Var.QUALS.index(self.__qual)+1}/{len(Var.QUALS)}</blockquote>
            ...
            await rep.report(f"Progress tracking failed: {str(e)}", "error")
